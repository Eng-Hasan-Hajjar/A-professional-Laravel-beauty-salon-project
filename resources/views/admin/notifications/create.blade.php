...
<div class="container text-right" dir="rtl">
    ...
    <form action="{{ route('notifications.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="title" class="form-control text-right @error('title') is-invalid @enderror" value="{{ old('title') }}" required>
            @error('title')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <textarea name="message" class="form-control text-right @error('message') is-invalid @enderror" required>{{ old('message') }}</textarea>
            @error('message')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <select name="type" class="form-control @error('type') is-invalid @enderror" required>
                <option value="appointment" {{ old('type') == 'appointment' ? 'selected' : '' }}>موعد</option>
                <option value="offer" {{ old('type') == 'offer' ? 'selected' : '' }}>عرض</option>
        ...
        <div class="mb-3">
            ...
            <select name="status" class="form-control @error('status') is-invalid @enderror" required>
                ...
                <option value="failed" {{ old('status') == 'failed' ? 'selected' : '' }}>فشل</option>
            </select>
            @error('status')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
