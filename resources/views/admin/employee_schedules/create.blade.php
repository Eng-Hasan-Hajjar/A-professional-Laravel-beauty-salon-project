    ...
    <form action="{{ route('employee-schedules.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="day_of_week" class="form-control @error('day_of_week') is-invalid @enderror" required>
                ...
                <option value="Monday" {{ old('day_of_week') == 'Monday' ? 'selected' : '' }}>الإثنين</option>
        ...
        <div class="mb-3">
            ...
            <input type="time" name="start_time" class="form-control text-right @error('start_time') is-invalid @enderror" value="{{ old('start_time') }}" required>
            @error('start_time')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="time" name="end_time" class="form-control text-right @error('end_time') is-invalid @enderror" value="{{ old('end_time') }}" required>
            @error('end_time')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
